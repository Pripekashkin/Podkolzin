...
begin
   ...
   begin
    if FNew then
      ...
      SQL.Text := 'execute procedure proc_report(' +IBQuery1.FieldByName('Номер').AsString + ', ''' +  Edit1.Text + ''', ''' + Edit2.Text + ''', '''+Edit3.Text+''')';
...
begin
   ...
   with IBQuery2 do
   begin
      SQL.Text := 'delete from store  where id = ' +IBQuery1.FieldByName('Номер').AsString;
    Transaction.StartTransaction;
    ExecSQL;
    Transaction.Commit;
    Transaction.Active := false;
